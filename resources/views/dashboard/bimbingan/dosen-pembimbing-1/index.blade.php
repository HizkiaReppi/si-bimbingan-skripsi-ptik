<x-dashboard-layout title="Bimbingan">
    ...
    <div class="card">
        <div class="d-flex justify-content-between align-items-center">
            <h5 class="card-header">Bimbingan Dengan Dosen Pembimbing {{ auth()?->user()?->student?->firstSupervisorFullname }}</h5>
            <div>
                @if ($guidances)
                <a href="{{ route('dashboard.bimbingan-1.print') }}" class="btn btn-secondary me-2">
                    <i class="fa fa-print me-2"></i>
                    Cetak Kartu Bimbingan
                </a>
                @endif
                <a href="{{ route('dashboard.bimbingan-1.create') }}" class="btn btn-primary me-4">Request Bimbingan</a>
            </div>
        </div>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item"
                                            href="{{ route('dashboard.bimbingan-1.show', $guidance->id) }}">
                                            <i class="bx bxs-user-detail me-1"></i> Detail
                                        </a>
                                        @if ($guidance->status_request == 'pending')
                                            <a class="dropdown-item"
                                                href="{{ route('dashboard.bimbingan-1.edit', $guidance->id) }}">
                                                <i class="bx bx-edit-alt me-1"></i> Edit
                                            </a>
                                            <a class="dropdown-item"
                                                href="{{ route('dashboard.bimbingan-1.destroy', $guidance->id) }}"
